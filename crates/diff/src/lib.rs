// ported from - https://github.com/codemod/codemod/blob/main/crates/core/src/diff.rs
...
#[napi(object)]
pub struct NapiDiffConfig {
    pub context_lines: Option<u32>,
    pub color: Option<bool>,
    pub max_lines_per_file: Option<u32>,
}

impl From<NapiDiffConfig> for DiffConfig {
    fn from(config: NapiDiffConfig) -> Self {
        DiffConfig {
            context_lines: config.context_lines.map(|v| v as usize).unwrap_or(3),
            color: config
                .color
                .unwrap_or_else(|| std::env::var("NO_COLOR").is_err()),
            max_lines_per_file: config.max_lines_per_file.map(|v| v as usize).unwrap_or(500),
        }
    }
}

#[napi(object)]
pub struct NapiFileDiff {
    pub path: String,
    pub diff_text: String,
    pub additions: u32,
    pub deletions: u32,
...
impl From<FileDiff> for NapiFileDiff {
...
pub async fn generate_diff(
    ...
    original: String,
    modified: String,
    config: Option<NapiDiffConfig>,
) -> Result<NapiFileDiff> {
    tokio::task::spawn_blocking(move || {
        let rust_config = config.map(DiffConfig::from).unwrap_or_default();
        let path = Path::new(&file_path);
        let result = generate_unified_diff(path, &original, &modified, &rust_config);
        Ok(NapiFileDiff::from(result))
    })
    .await
